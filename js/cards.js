  ...
  await fetch("https://api.covid19api.com/summary")
    ...
    .then((data) => {
      ...
      document.getElementById(
        "confirmed"
      ).innerHTML += `<p>${confirmed}<br><small>${c}${new_confirmed}</small></p>`;
      ...
      document.getElementById(
        "recovered"
      ).innerHTML += `<p>${recovered}<br><small>${c}${new_recovered}</small></p>`;
      ...
      document.getElementById(
        "deceased"
      ).innerHTML += `<p>${deceased}<br><small>${c}${new_deceased}</small></p>`;
    });
}
...
>>>>>>> 14d0ae67732223a391877d41edcb1574ac97dde1
